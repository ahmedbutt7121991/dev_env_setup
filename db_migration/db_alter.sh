#!/bin/bash
source config.sh
BD_HOST="$( awk '/host/ {print $3}' $db_cred_file_path)"
DB_NAME="$( awk '/dbname/ {print $3}' $db_cred_file_path)"
DB_PASSWORD="$( awk '/password/ {print $3}' $db_cred_file_path)"
USER_NAME="$( awk '/username/ {print $3}' $db_cred_file_path)"
