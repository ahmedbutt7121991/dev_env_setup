...
dev_task() {
    ...
    then
        ...
        echo "Downloading Data base: wget http://qa.tilemountain.co.uk/pub/db.sql.gz"
        wget http://qa.tilemountain.co.uk/pub/db.sql.gz
        # mv db.sql.gz var/
        cd ..
        pwd
        echo y | bash mage2-dbdump.sh -rz
...
mysql -h${BD_HOST} -u${USER_NAME} -p${DB_PASSWORD} -D ${DB_NAME} -e "Update ${DB_TABLE} set value='http://qa1.tilemountain.co.uk/' Where path='web/unsecure/base_url';"
# mysql -h${BD_HOST} -u${USER_NAME} -p${DB_PASSWORD} -D ${DB_NAME} -e "Update ${DB_TABLE} set value='http://qa1.tilemountain.co.uk/' Where path='web/unsecure/base_media_url';"
mysql -h${BD_HOST} -u${USER_NAME} -p${DB_PASSWORD} -D ${DB_NAME} -e "Update ${DB_TABLE} set value='http://qa1.tilemountain.co.uk/' Where path='web/unsecure/base_link_url';"
mysql -h${BD_HOST} -u${USER_NAME} -p${DB_PASSWORD} -D ${DB_NAME} -e "Update ${DB_TABLE} set value='http://qa1.tilemountain.co.uk/' Where path='web/secure/base_url';"
# mysql -h${BD_HOST} -u${USER_NAME} -p${DB_PASSWORD} -D ${DB_NAME} -e "Update ${DB_TABLE} set value='http://qa1.tilemountain.co.uk/' Where path='web/secure/base_media_url';"
mysql -h${BD_HOST} -u${USER_NAME} -p${DB_PASSWORD} -D ${DB_NAME} -e "Update ${DB_TABLE} set value='http://qa1.tilemountain.co.uk/' Where path='web/secure/base_link_url';"
mysql -h${BD_HOST} -u${USER_NAME} -p${DB_PASSWORD} -D ${DB_NAME} -e "Update ${DB_TABLE} set value='http://qa1.tilemountain.co.uk/' Where path='tm_integrations/general/tm_url';"
mysql -h${BD_HOST} -u${USER_NAME} -p${DB_PASSWORD} -D ${DB_NAME} -e "Update ${DB_TABLE} set value='http://qa1.tilemountain.co.uk/' Where path='wallfloor/general/wallfloor_url';"
mysql -h${BD_HOST} -u${USER_NAME} -p${DB_PASSWORD} -D ${DB_NAME} -e "Update ${DB_TABLE} set value='http://qa1.tilemountain.co.uk/' Where path='tonssystem/general/tons_url';"
